...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        Admins can update role and supervisor (PRD section 5.4).
        """
        request_user = self.context['request'].user
        is_admin = request_user.role == User.Role.ADMIN

        # Only admins can update role, is_active, or supervisor
        if not is_admin:
            ...
            if 'supervisor' in validated_data and validated_data['supervisor'] != instance.supervisor:
                raise serializers.ValidationError({"supervisor": "Only admins can change supervisors"})
...
class UserCreationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['email', 'full_name', 'role', 'supervisor', 'password', 'confirm_password', 'is_active']
