...
const progressRatingOptions = [
  { label: 'On Track', value: 'on_track', icon: 'check_circle', color: 'positive', description: 'Progressing as expected' },
  { label: 'At Risk', value: 'at_risk', icon: 'warning', color: 'orange', description: 'May need assistance' },
  { label: 'Behind', value: 'behind', icon: 'error', color: 'negative', description: 'Behind schedule' },
  { label: 'Completed Early', value: 'completed_early', icon: 'fast_forward', color: 'primary', description: 'Finished ahead of schedule' }
]
...
const loadReport = async () => {
  ...
  try {
    ...
    $q.notify({ color: 'negative', message: 'Failed to load report', icon: 'error' })
    router.back()
  } finally {
    loading.value = false
  }
...
const handleSubmitClick = async () => {
  if (!isValidForSubmit.value) {
    $q.notify({ color: 'warning', message: 'Please fill in all required fields (*) before submitting', icon: 'warning' })
    return
  }
