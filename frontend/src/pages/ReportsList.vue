        ...
        >
          ...
          <template v-slot:body-cell-status="props">
            <q-td :props="props" align="center">
              <div class="row items-center justify-center q-gutter-xs">
                ...
                <q-badge v-if="props.row.is_late" color="negative" label="Late" class="status-badge" />
              </div>
            </q-td>
          </template>
          ...
          <template v-slot:body-cell-actions="props">
            <q-td :props="props" class="actions-cell">
              ...
              <q-btn
                v-if="props.row.status === 'submitted' && isSupervisor"
                flat
                dense
                round
                icon="verified"
          ...
          <template v-slot:no-data>
            <div class="full-width text-center q-pa-xl text-grey-6">
              <q-icon name="assignment" size="4rem" color="grey-4" class="q-mb-md" />
              <div class="text-h6 text-weight-bold text-grey-8">No reports found</div>
              <div class="text-subtitle2">{{ emptyDescription }}</div>
              <q-btn v-if="isEmployee" color="primary" label="Create Report" icon="add" @click="createNewReport" class="q-mt-md" unelevated />
              <q-btn v-else color="primary" label="Refresh" icon="refresh" @click="loadReports" class="q-mt-md" unelevated />
            </div>
          </template>
        </q-table>
      </q-card>
...
const viewModeOptions = computed(() => {
  const options = []
  if (isEmployee.value || isSupervisor.value) options.push({ label: 'My Reports', value: 'mine' })
...
const viewTitle = computed(() => {
  if (viewMode.value === 'all') return 'All Organization Reports'
  if (viewMode.value === 'team') return 'Team Report Dashboard'
  return 'Your Weekly Reports'
})

const emptyDescription = computed(() => {
  if (isEmployee.value) return 'You haven\'t created any reports yet. Start tracking your progress!'
  if (isSupervisor.value) return 'Your team hasn\'t submitted any reports yet.'
  return 'The system has no report data yet.'
})
...
const loadReports = async () => {
  ...
  try {
    ...
    if (route.query.employee) {
      params.employee = route.query.employee
    }

    if (viewMode.value === 'all') await reportsStore.fetchAllReports(params)
    else if (viewMode.value === 'team') await reportsStore.fetchTeamReports(params)
    else await reportsStore.fetchMyReports(params)
  } catch (error) {
...
const viewReport = (evt, row) => router.push(`/reports/${row.id}`)
...
onMounted(() => {
  ...
  if (route.query.view === 'team' && isSupervisor.value) {
    viewMode.value = 'team'
  } else if (route.query.review === '1' && (isSupervisor.value || isAdmin.value)) {
    viewMode.value = 'team'
