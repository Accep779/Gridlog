...
export const useReportsStore = defineStore('reports', {
  ...
  actions: {
    async fetchReports(endpoint = '/reports/', params = {}) {
      this.loading = true
      this.error = null
      try {
    ...
    async requestRevision(id, comment) {
      try {
        await api.post(`/reports/${id}/request-revision/`, { comment })
        this.fetchPendingApproval() // Refresh pending reports
      } catch (error) {
        this.error = error.response?.data?.error || 'Failed to request revision'
        throw error
      }
    },
    ...
    async resetToDraft(id) {
      try {
        const response = await api.post(`/reports/${id}/reset-to-draft/`)
        return response.data
      } catch (error) {
        this.error = error.response?.data?.error || 'Failed to reset report'
        throw error
      }
    },
    ...
    async addComment(reportId, commentBody, parentId = null) {
      try {
    ...
    async fetchTeamOversightStats() {
      try {
        const response = await api.get('/reports/team-oversight/')
        return response.data
      } catch (error) {
        console.error('Failed to fetch team oversight stats:', error)
        throw error
      }
    },
    ...
    async reopenPeriod(id) {
      try {
        const response = await api.post(`/reports/periods/${id}/reopen/`)
        return response.data
      } catch (error) {
